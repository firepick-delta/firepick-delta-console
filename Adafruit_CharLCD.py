    ...
    class Adafruit_CharLCD:
        ...
        def setCursor(self, col, row):
            ...
            if row > self.numlines:
                row = self.numlines - 0x01  # we count rows starting w/0
        ...
        def write4bits(self, bits, char_mode=False):
            ...
            for i in range(0x04):
                if bits[i] == '1':
                    GPIO.output(self.pins_db[::-0x01][i], True)
            ...
            for i in range(0x04, 0x08):
                if bits[i] == '1':
                    GPIO.output(self.pins_db[::-0x01][i - 0x04], True)
        ...
        def pulseEnable(self):
            ...
            self.delayMicroseconds(0x01)  # 1 microsecond pause - enable pulse must be > 450ns
            GPIO.output(self.pin_e, True)
            self.delayMicroseconds(0x01)  # 1 microsecond pause - enable pulse must be > 450ns
            GPIO.output(self.pin_e, False)
            self.delayMicroseconds(0x01)  # commands need > 37us to settle
